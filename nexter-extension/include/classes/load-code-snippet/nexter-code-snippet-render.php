	...
	class Nexter_Builder_Code_Snippets_Render {
		...
		public function create_new_snippet(){
			...
			if ( isset( $_POST['title'] ) ) {
				...
				wp_send_json_success(
					[
					'id'      => $id,
					'message' => __( 'Snippet Created Successfully.', 'nexter-extension' )
					 ] 
				);
				/* if ($post_id) {
		...
		public function migrate_post_snippets_to_file_based() {
			...
			if ( $query->have_posts() ) {
				while ( $query->have_posts() ) {
					...
					$metaData['condition'] = array_filter(
						$metaData['condition'],
						function($value) {
		...
		* */
			...
			if ( $post_id ) {
				...
				if ( $post && $post->post_type === self::$snippet_type ) {
					...
					wp_send_json_success(
						[
						'status'  => ! $get_status,
						'message' => __( 'Updated Status Successfully', 'nexter-extension' )
						 ] 
					);
				} else {
					wp_send_json_error( ['message' => __( 'Invalid post or post type', 'nexter-extension' )] );
			...
			if ( isset( $_POST['post_id'] ) && ! empty( $_POST['post_id'] ) ) {
				...
				if ( $file_based && ! empty( $post_id ) ) {
					...
					if ( ! empty( $file_code_list ) ) {
						...
						// Perform location migration if needed
						$migrated_location          = $this->migrate_location_if_needed(
							$post_id,
							$type,
							$current_location, 
		...
		public static function nexter_code_html_hooks_actions() {
			...
			$css_locations = apply_filters(
				'nexter_get_css_selector_locations',
				[
				'before_html_element',
				'after_html_element', 
				'start_html_element',
