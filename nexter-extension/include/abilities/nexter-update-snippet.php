...
wp_register_ability(
	...
	'input_schema'        => [
		...
		'properties'           => [
			...
			'name'               => [
			'type'        => 'string',
			'description' => 'Updated name/title.'
			],
			'code'               => [
			'type'        => 'string',
			'description' => 'Updated source code. For PHP, do NOT include <?php tag.'
			],
			'type'               => [
			'type'        => 'string',
			...
			'location'           => [
			'type'        => 'string',
			'description' => 'Updated insertion location.'
			],
			'code_execute'       => [
			'type'        => 'string',
			...
			'include_rules'      => [
			'type'        => 'array',
			'description' => 'Include display rules.'
			],
			'exclude_rules'      => [
			'type'        => 'array',
			'description' => 'Exclude display rules.'
			],
			'php_hidden_execute' => [
			'type' => 'string',
			'enum' => ['yes', 'no']
...
function nexter_mcp_update_snippet(array $input): array {
	if ( ! class_exists( 'Nexter_Code_Snippets_File_Based' ) ) {
		return [
		'success' => false,
		'error'   => 'Nexter Extension code snippets not available.'
		];
	}
	...
	if ( empty( $data ) || ! isset( $data['meta'] ) ) {
		return [
		'success' => false,
		'error'   => 'Failed to parse snippet metadata.'
		];
	}
	...
	if ( $type === 'php' && preg_match( '/^<\?php/', $code ) ) {
		return [
		'success' => false,
		'error'   => 'PHP code must NOT start with <?php — it is added automatically.'
		];
	}
