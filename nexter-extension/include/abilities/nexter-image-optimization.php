...
wp_register_ability(
	'nexter/update-image-optimization',
	[
	...
	'description' => 'Enable/disable image optimization.'
			...
			'enum'        => ['webp', 'original', 'smart', 'avif']
			],
			'quality_mode'      => [
			'type'        => 'string',
			'description' => 'Quality preset.',
			'enum'        => ['balanced', 'lossless', 'aggressive']
			],
			'max_width'         => [
			'type'        => 'integer',
			...
			'auto_convert'      => [
			'type'        => 'boolean',
			'description' => 'Auto convert on upload.'
			],
			'exif_data'         => [
			'type'        => 'string',
			...
			'avoid_larger'      => [
			'type'        => 'boolean',
			'description' => 'Skip if optimized is larger.'
			],
			'exclude_paths'     => [
			'type'        => 'array',
			...
			'run_in_background' => [
			'type'        => 'boolean',
			'description' => 'Process in background.'
			],
			'exclude_png_webp'  => [
			'type'        => 'boolean',
			'description' => 'Exclude PNG from WebP conversion.'
			],
			'exclude_png_avif'  => [
			'type'        => 'boolean',
			'description' => 'Exclude PNG from AVIF conversion.'
			],
		],
		'additionalProperties' => false,
	],
...
function nexter_mcp_update_image_optimization(array $input): array {
	...
	foreach ( $string_keys as $k ) {
		if ( isset( $input[ $k ] ) ) {
			$values[ $k ] = sanitize_text_field( $input[ $k ] ); }
	}
	foreach ( $int_keys as $k ) {
		if ( isset( $input[ $k ] ) ) {
			$values[ $k ] = (int) $input[ $k ]; }
	}
	foreach ( $bool_keys as $k ) {
		if ( isset( $input[ $k ] ) ) {
			$values[ $k ] = (bool) $input[ $k ]; }
	}
	...
	return [
	'success' => true,
	'message' => 'Image optimization settings updated.'
	];
}
