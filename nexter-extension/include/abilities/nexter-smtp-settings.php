...
wp_register_ability(
	...
	'description' => 'Enable/disable SMTP.'
			...
			'host'       => [
			'type'        => 'string',
			'description' => 'Custom SMTP host.'
			],
			'port'       => [
			'type'        => 'integer',
			'description' => 'Custom SMTP port (25, 465, 587).'
			],
			'encryption' => [
			'type'        => 'string',
			...
			'auto_tls'   => [
			'type'        => 'boolean',
			'description' => 'Enable STARTTLS auto-negotiation.'
			],
			'auth'       => [
			'type'        => 'boolean',
...
function nexter_mcp_update_smtp_settings(array $input): array {
	...
	if ( isset( $input['type'] ) ) {
		$values['type'] = sanitize_text_field( $input['type'] ); }
	if ( isset( $input['from_name'] ) ) {
		$values['from_name'] = sanitize_text_field( $input['from_name'] ); }
	if ( isset( $input['from_email'] ) ) {
		$values['from_email'] = sanitize_email( $input['from_email'] ); }

	// Custom SMTP fields
	if ( ($values['type'] ?? '') === 'custom' || isset( $input['host'] ) ) {
		$custom = $values['custom'] ?? [];
		if ( isset( $input['host'] ) ) {
			$custom['host'] = sanitize_text_field( $input['host'] ); }
		if ( isset( $input['port'] ) ) {
			$custom['port'] = (int) $input['port']; }
		if ( isset( $input['encryption'] ) ) {
			$custom['encryption'] = sanitize_text_field( $input['encryption'] ); }
		if ( isset( $input['auto_tls'] ) ) {
			$custom['autoTLS'] = (bool) $input['auto_tls']; }
		if ( isset( $input['auth'] ) ) {
			$custom['auth'] = (bool) $input['auth']; }
		if ( isset( $input['username'] ) ) {
			$custom['username'] = sanitize_text_field( $input['username'] ); }
		if ( isset( $input['password'] ) ) {
			$custom['password'] = $input['password']; }
