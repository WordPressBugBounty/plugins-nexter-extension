...
wp_register_ability(
	...
	'output_schema'       => [
		...
		'properties' => [
			...
			'id'      => [
	'type'        => 'string',
	'description' => 'The ID of the newly created snippet.'
			],
			'message' => ['type' => 'string'],
		],
	],
...
function nexter_mcp_create_snippet(array $input): array {
	...
	if ( ! in_array( $type, ['php', 'css', 'javascript', 'htmlmixed'], true ) ) {
		return [
		'success' => false,
		'error'   => 'Invalid type. Must be: php, css, javascript, or htmlmixed.'
		];
	}
	...
	if ( empty( $storage_dir ) ) {
		return [
		'success' => false,
		'error'   => 'Snippet storage directory not available.'
		];
	}
