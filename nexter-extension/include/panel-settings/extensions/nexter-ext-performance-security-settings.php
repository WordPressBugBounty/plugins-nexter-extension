...
class Nexter_Ext_Performance_Security_Settings {
    ...
    public function __construct() {
		...
		if( !empty($extension_option) ){
			...
			if( isset($extension_option['image-upload-optimize']) && !empty($extension_option['image-upload-optimize']['switch'])){
				...
				// Show notice when image optimization is not enabled (only on Media page and Media popup)
				add_action( 'admin_notices', array( $this, 'nexter_image_optimize_notice' ) );
			}
		}
	...
	/**
	 * Display admin notice when image optimization is not enabled
	 * Only shows on Media page and Media popup
	 */
	...
	/**
	 * Render the image optimize notice HTML
	 */
	 ...
	 */
	private function get_image_optimize_notice_html() {
		$notice_title = esc_html__( 'Free Image Compression Is Now Available In Nexter - No Recurring Cost or APIs Required', 'nexter-extension' );
		$notice_desc = esc_html__( 'Enable image compression to reduce image size, convert to WebP, AVIF, save storage space, and improve website performance. without any third-party plugins, APIs, or recurring costs.', 'nexter-extension' );
