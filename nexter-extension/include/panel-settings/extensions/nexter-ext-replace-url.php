...
if ( ! function_exists( 'nxt_replace_url' ) ) {
	function nxt_replace_url() {
		...
		if ( ! empty( $user ) && isset( $user->roles ) && array_intersect( $allowed_roles, $user->roles ) ) {
			// wp_unslash() already strips the slashes WP adds to superglobals (stripslashes_deep) —
			// don't wrap it in a second stripslashes(), which strips one level too many from URLs
			// containing backslash sequences. sanitize_text_field() is intentionally avoided because
			// it removes <, >, and HTML-like chars that can be legitimate in a URL.
			$from = ( isset( $_POST['from'] ) && '' !== $_POST['from'] ) ? wp_unslash( $_POST['from'] ) : '';
			$to   = ( isset( $_POST['to'] ) && '' !== $_POST['to'] ) ? wp_unslash( $_POST['to'] ) : '';
			...
			$selTables = is_array( $selTables ) ? array_map(
				function( $table ) {
...
if ( ! function_exists( 'nxt_replace_confirm_url' ) ) {
	function nxt_replace_confirm_url() {
		...
		if ( ! empty( $user ) && isset( $user->roles ) && array_intersect( $allowed_roles, $user->roles ) ) {
			...
			// containing backslash sequences. sanitize_text_field() is intentionally avoided because
			// it removes <, >, and HTML-like chars that can be legitimate in a URL.
			$from = ! empty( $_POST['from'] ) ? wp_unslash( $_POST['from'] ) : '';
			$to   = ! empty( $_POST['to'] ) ? wp_unslash( $_POST['to'] ) : '';
