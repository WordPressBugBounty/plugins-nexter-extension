    ...
    function nxt_recaptch_render($reData){
        ...
        if ( ! wp_script_is( 'nexter_recaptcha_api', 'registered' ) ) {
            ...
            if ( isset(  $reData['version'] ) && 'v3' ==  $reData['version'] ) {
                ...
                wp_register_script( 'nexter_recaptcha_api', $api_url, NEXTER_EXT_VER, true );
            }
...
if ( ! function_exists( 'nxt_login_check' ) ) {
    function nxt_login_check($user){
        ...
        if ( ! $nxrecap_check['response'] ) {
			...
			$errors = new WP_Error( $error_code, __( 'Authentication failed.', 'nexter-ext' ) );
        ...
        if ( isset( $reoption['version'] ) && in_array( $reoption['version'], array( 'v2', 'invisible', 'v3' ) ) ) {
			if ( ! isset( $_POST["g-recaptcha-response"] ) ) {
				...
				if ( empty( $response ) ) {
					$result = array(
							'response' => false,
							'reason' => $debug ? __( 'Response is empty', 'nexter-ext' ) : 'VERIFICATION_FAILED'
						);
...
if ( ! function_exists( 'nxttch_get_error_message' ) ) {
    function nxttch_get_error_message( $message_code = 'incorrect', $display = false ){
		...
		$errormsg = array(
			'missing-input-secret' 		=> __( 'Secret Key is missing.', 'nexter-ext' ),
			'invalid-input-secret' 		=> sprintf( '<strong>%s</strong> <a target="_blank" href="https://www.google.com/recaptcha/admin#list" rel="noopener noreferrer" >%s</a> %s.', __( 'Secret Key is invalid.', 'nexter-ext' ),__( 'Check your domain configurations', 'nexter-ext' ),__( 'and enter it again', 'nexter-ext' )
			),
			'incorrect'					=> __( 'You have entered an incorrect reCAPTCHA value.', 'nexter-ext' ),
			'multiple_blocks'			=> __( 'More than one reCAPTCHA has been found in the current form. Please remove all unnecessary reCAPTCHA fields to make it work properly.', 'nexter-ext' ),
            'incorrect-captcha-sol'		=> __( 'User response is invalid', 'nexter-ext' ),
             'RECAPTCHA_SMALL_SCORE'     => __( 'reCaptcha v3 test failed', 'nexter-ext' ),
			'RECAPTCHA_EMPTY_RESPONSE'	=> __( 'User response is missing.', 'nexter-ext' ),
            'ERROR_WRONG_SECRET'		=> __( 'You have entered incorrect  secret key.', 'nexter-ext' ),
		);
...
if( ! function_exists( 'nxt_comment_check' ) ){
    function nxt_comment_check() {
        ...
        if ( empty($nxtcptch_check['response']) ) {
            ...
            $error_message = sprintf( '<strong>%s</strong>:&nbsp;%s&nbsp;%s', __( 'Error', 'nexter-ext' ), $message, __( 'Click the BACK button on your browser and try again.', 'nexter-ext' )
            );
            ...
            $content .= '<div>';
                $content .= '<span>';
                    if( isset( $reoption['version'] ) && !empty($reoption['version'] ) && ( $reoption['version'] == 'invisible' || $reoption['version'] == 'v3' ) ){
                        $content .=  sprintf( '%s', __( 'Please submit "Test verification" ', 'nexter-ext' ) );
                    }else{
                        $content .=  sprintf( '%s', __( 'Please complete the captcha and submit "Test verification"', 'nexter-ext' ) );
                    }
                $content .= '</span>';
                $content .= '<button id="nxt_verfiy_btn" class="nxt-test-recaptch">';
                    ...
                    $content .= esc_html__( 'Test reCAPTCHA', 'nexter-ext' );
                $content .=  '</button>';
            $content .= '</div>';
		...
		if ( !empty($user) && isset($user->roles) && array_intersect( $allowed_roles, $user->roles ) ) {
            ...
            if( wp_verify_nonce( $nonce , 'nexter_recaptch_verify' ) ) {
                ...
                if ( empty($nxtcptch_check['response']) ) {
                    ...
                    $cnt .= '<div class="updated nxt-verify-resul"><p>'.esc_html__('The verification is successfully completed.','nexter-ext' ).'</p></div>';
                }
            }
        }else{
            $cnt .= '<div class="error nxt-verify-result"><p>'.esc_html__('Only Admin can run this.','nexter-ext' ).'</p></div>';
        }
        wp_send_json_success(
            array(
                'content'	=> $cnt,
