...
class Nexter_Ext_Image_Upload_Optimization {
	...
	public function get_settings() {
		...
		$option = Nxt_Options::performance() ?: array();
	...
	private static function calculate_dimensions( $width, $height, $max_width, $max_height ) {
		return Nxt_Image_Processor::calculate_dimensions( $width, $height, $max_width, $max_height );
	}
	...
	/** @var Nxt_Image_Processor Image processing sub-class. */
	private $processor;

	/** @var Nxt_Image_Frontend_Replacement Frontend URL replacement sub-class. */
	private $frontend_replacement;
	...
	public function __construct() {
		...
		// Instantiate extracted sub-classes.
		require_once plugin_dir_path( __FILE__ ) . 'class-nxt-image-processor.php';
		require_once plugin_dir_path( __FILE__ ) . 'class-nxt-image-frontend-replacement.php';
		$this->processor            = new Nxt_Image_Processor( $this );
		$this->frontend_replacement = new Nxt_Image_Frontend_Replacement( $this );
		...
		// Direct Replacement : replace img/background URLs in content with .webp/.avif (delegated to frontend replacement).
		add_action( 'init', array( $this->frontend_replacement, 'register_direct_replacement_hooks' ), 20 );
	...
	public function register_direct_replacement_hooks() {
		$this->frontend_replacement->register_direct_replacement_hooks();
	}

	public function start_direct_replacement_buffer() {
		$this->frontend_replacement->start_direct_replacement_buffer();
	}
	...
	public function replace_img_with_webp( $content ) {
		return $this->frontend_replacement->replace_img_with_webp( $content );
	}
	...
	public function filter_global_buffer( $buffer ) {
		return $this->frontend_replacement->filter_global_buffer( $buffer );
	}
	...
	public function process_image_original( $file_path, $settings ) {
		return $this->processor->process_image_original( $file_path, $settings );
	}
	...
	public function process_image( $file_path, $settings ) {
		return $this->processor->process_image( $file_path, $settings );
	}
	...
	private function fix_imagick_orientation( $imagick ) {
		// Delegated to Nxt_Image_Processor.
	}
	 ...
	 * @return string|false
	 */
	public function path_to_url( $file_path ) {
		$upload_dir = self::get_upload_dir();
		$basedir    = wp_normalize_path( $upload_dir['basedir'] );
		$file_path  = wp_normalize_path( $file_path );
