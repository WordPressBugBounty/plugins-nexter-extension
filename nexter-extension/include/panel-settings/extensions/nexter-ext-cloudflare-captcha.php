...
class Nexter_Ext_Cloudflare_Captcha {
	...
	public function validate_turnstile_nexter_form( $default, $captcha_value ) {
		...
		if ( empty( $captcha_value ) ) {
			return [
			'success' => false,
			'data'    => esc_html__( 'Invalid CAPTCHA validation response format.', 'nexter-extension' )
			];
		}
		...
		if ( empty( $response['success'] ) || $response['success'] !== true ) {
			return [
			'success' => false,
			'data'    => esc_html__( 'Please verify that you are human.', 'nexter-extension' )
			];
		}

		return [
		'success' => true,
		'data'    => esc_html__( 'Cloudflare Turnstile validated successfully.', 'nexter-extension' )
		];
	}
