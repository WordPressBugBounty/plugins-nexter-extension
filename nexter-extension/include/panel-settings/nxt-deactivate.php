...
if ( ! class_exists( 'Nxt_Ext_Deactivate' ) ) {
	...
	class Nxt_Ext_Deactivate {
		...
		public function nxt_check_white_label(){
			// Suppress the Nexter/POSIMYTH-branded deactivation popup when the site has been
			// white-labelled via Pro. A rebranded install must never surface POSIMYTH branding
			// (or phone api.posimyth.com from the survey). Returns false to hide the popup.
			if ( ( defined( 'NXT_PRO_EXT' ) || defined( 'TPGBP_VERSION' ) ) && class_exists( 'Nxt_Options' ) ) {
				$label_options = Nxt_Options::white_label();
				if ( is_array( $label_options ) && ! empty( $label_options['brand_name'] ) ) {
					return false;
				}
			}
			return true;
					...
					<div class="nxt-ext-modal-body">
						...
						<form class="nxt-ext-feedback-dialog-form" method="post">
							...
							<div class="nxt-ext-help-link">
								...
								<span> <a href="<?php if ( defined( 'NXT_PRO_EXT_VER' ) ) {
									echo esc_url( 'https://store.posimyth.com/helpdesk/?utm_source=wpbackend&utm_medium=admin&utm_campaign=links' ); } else {
													echo esc_url( 'https://wordpress.org/support/plugin/nexter-extension/' ); }  ?>" target="_blank" rel="noopener noreferrer" > <?php echo esc_html__( 'Create A Ticket.', 'nexter-extension' ) ?> </a> <?php echo esc_html__( 'We reply within 24 working hours.', 'nexter-extension' ); ?></span>                                 
