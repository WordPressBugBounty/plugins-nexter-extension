...
class Nxt_Panel_Ajax_Router {
	...
	public function nexter_ext_save_data_ajax(){
		...
		if ( ! empty( $ext ) && $ext === 'custom-upload-font' && ! empty( $fonts ) ) {
			...
			if ( ! empty( $get_option ) && isset( $get_option[ $ext ] ) ) {
				// Security: Safe JSON decode with validation
				$decoded_adminHide = $this->parent->safe_json_decode( $adminHide, false );
				if ( is_array( $decoded_adminHide ) ) {
					$get_option[ $ext ]['values'] = $decoded_adminHide;
					update_option( $option_page, $get_option );
				}
			}
			wp_send_json_success();
			...
			if ( ! empty( $get_option ) && isset( $get_option[ $ext ] ) ) {
				// Security: Safe JSON decode with validation
				$decoded_adminbarClean = $this->parent->safe_json_decode( $adminbarClean, false );
				if ( is_array( $decoded_adminbarClean ) ) {
					$get_option[ $ext ]['values'] = $decoded_adminbarClean;
					update_option( $option_page, $get_option );
				}
			}
			wp_send_json_success();
	...
	*/
	...
	public function nexter_ext_theme_install_ajax(){
		...
		if ( is_wp_error( $response ) ) {
			...
			// Security: Safe unserialize with error checking. Restrict to stdClass — the wp.org info
			// API returns a serialized stdClass, so allowing it keeps the is_object()/->name/
			// ->download_link checks working while blocking arbitrary-class object injection.
			$body       = wp_remote_retrieve_body( $response );
			$theme_info = unserialize( $body, array( 'allowed_classes' => array( 'stdClass' ) ) );
			...
			// Security: verify the package downloaded successfully before writing it. Without this a
			// transport error, non-200 response, or empty body would write a corrupt/empty ZIP and
			// still report success.
			if ( is_wp_error( $theme ) || (int) wp_remote_retrieve_response_code( $theme ) !== 200 || '' === wp_remote_retrieve_body( $theme ) ) {
				wp_send_json_error( array( 'content' => __( 'Could not download the theme package.', 'nexter-extension' ) ) );
			}
			...
			$wp_filesystem->put_contents( WP_CONTENT_DIR.'/themes/'.$theme_slug . '.zip', wp_remote_retrieve_body( $theme ) );
			$zip = new ZipArchive();
	...
	public function nexter_ext_plugin_install_ajax(){
		...
		// ->download_link checks working while blocking arbitrary-class object injection.
		$body        = wp_remote_retrieve_body( $response );
		$plugin_info = unserialize( $body, array( 'allowed_classes' => array( 'stdClass' ) ) );
